...
import "../../src/competitions/JudgeCompetitionMultipleWinners.sol";
import {CompetitionV1} from "../../src/CompetitionV1.sol";

contract JudgeCompetitionMultipleWinnersTest is BaseTest {
    ...
    function testEndCompetitionMultipleWinners() public {
        vm.startPrank(judge);
        (, uint256 competitionId,) = createCompetition();
        vm.stopPrank();
        uint256 pubIdOne = enterCompetition(competitionId);
        uint256 pubIdTwo = enterCompetition(competitionId, pubIdOne);

        skip(1 days + 1 seconds);

        PublicationId[] memory publicationIds = new PublicationId[](2);
        publicationIds[0] = PublicationId({profileId: userOneProfileId, pubId: pubIdOne});
        publicationIds[1] = PublicationId({profileId: userOneProfileId, pubId: pubIdTwo});

        vm.startPrank(judge);
        endCompetition(competitionId, publicationIds);
        vm.stopPrank();

        PublicationId[] memory winners = lensCompetitionHub.winners(competitionId);
        assertEq(winners[0].profileId, userOneProfileId);
        assertEq(winners[0].pubId, pubIdOne);
        assertEq(winners[1].profileId, userOneProfileId);
        assertEq(winners[1].pubId, pubIdTwo);
    }
    ...
    {
        JudgeCompetitionMultipleWinnersInitData memory competitionInit =
            JudgeCompetitionMultipleWinnersInitData({judge: judge});
    ...
    function createCompetition() internal returns (uint256, uint256, CompetitionV1) {
        ...
        return lensCompetitionHub.createCompetition(
            address(judgeCompetitionMultipleWinners), block.timestamp + 1 days, postWithSigData, competitionInitData
        );
    }
    ...
    function enterCompetition(uint256 competitionId) internal returns (uint256) {
        return enterCompetition(competitionId, 0);
    }

    function enterCompetition(uint256 competitionId, uint256 previousPubId) internal returns (uint256 pubId) {
        (DataTypes.CommentWithSigData memory commentWithSigData, bytes memory competitionEnterData) =
            enterCompetitionParams(1, 1);

        vm.expectEmit(true, true, true, true, address(lensCompetitionHub));
        emit Events.EnteredCompetition(competitionId, userOneProfileId, previousPubId + 1);
    ...
    function endCompetitionParams(uint256 profileId, uint256 pubId) internal pure returns (bytes memory) {
        PublicationId[] memory publicationIds = new PublicationId[](1);
        publicationIds[0] = PublicationId({profileId: profileId, pubId: pubId});

        return endCompetitionParams(publicationIds);
    }

    function endCompetitionParams(PublicationId[] memory publicationIds) internal pure returns (bytes memory) {
        JudgeCompetitionMultipleWinnersEndData memory competitionEnd =
            JudgeCompetitionMultipleWinnersEndData({publicationIds: publicationIds});
    ...
    function endCompetition(uint256 competitionId, PublicationId[] memory publicationIds) internal {
        bytes memory competitionEndData = endCompetitionParams(publicationIds);
